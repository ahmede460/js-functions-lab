...
function isCharAVowel(singleCharacter){
    singleCharacter = singleCharacter.toLowerCase()
...
console.log('Exercise 6 Result:', maxOfThree(30, 10, 12));
...
function convertTemperature(temp, tempScale){
    if (tempScale === "C"){
        return `${parseInt(temp) * 1.8 + 32} (Fahrenheit).`
    } 
    else if (tempScale === "F"){
        return `${(parseInt(temp) - 32) * 1.8} (Celsuis).`
    }
}
...
function calculateGrade(score){if (score >= 90){
    return "A"
} 
